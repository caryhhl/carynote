...
private[spark] class Master(
  ...
  override def receiveWithLogging = {
    ...
    case RegisterApplication(description) => {
      ...
      if (state == RecoveryState.STANDBY) {
        ...
        // 注册Application
        // 将ApplicationInfo加入缓存，将Applcation加入等待调度的队列——waitingApps
  ...
  private def schedule() {
    ...
    // 首先呢，Application的调度算法有两种，一种是spreadOutApps，另一种是非spreadOutApps
    if (spreadOutApps) {
      // Try to spread out each app among all the nodes, until it has all its cores7
      ...
      for (app <- waitingApps if app.coresLeft > 0) {
        ...
        // 给每个worker分配完applcation要求的cpu core之后
        // 遍历worker
        for (pos <- 0 until numUsable) {
          ...
          if (assigned(pos) > 0) {
            ...
            // 因为，我们这里是基于总的cpu来分配的，就是说，比如要求3个executor，每个要3个cpu，那么比如，有9个
            // worker，每个有1个cpu，那么其实总共知道，要分配9个core，其实根据这种算法，会给每个worker分配一个
      ...
      // 每个appliation，都尽可能分配到尽量少的worker上去，比如总共有10个worker，每个有10个core
      ...
      // 但是在这种算法下，其实总共只会启动2个executor，每个有10个core把
